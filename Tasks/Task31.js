...
export default function Task31() {
    ...
    const imageMargin = 30 ; // from styles.pressable margin: 15
    const scrolls = new Array(image.length);
    
    
    for (let i = 0 ; i < image.length ; i++){
        size[i] = Image.resolveAssetSource(image[i][0]);
    ...
    const Item = ({ item, index }) => {
        ...
        const size = Image.resolveAssetSource(item);
        const newHeight = size.height * ( windowWidth / size.width ); ;
        ...
        >
            ...
            <Image source={item} style={[styles.images, { width: windowWidth, height: newHeight }]} />
        </Pressable>
    ...
    const RowItem = ({ item, index }) => (
        <View style={{ flex: 1}}>
            ...
            <View style={{flexDirection: 'row', justifyContent: 'space-between', position: 'absolute',width: '100%'}}>
                ...
                <Pressable style={{}}>
                    <Icon name="add" size={22} color="blue"  style={{bottom : 5 , left: 10, }}
                    onPress={() =>{
                        const updatedArray = image.map((rowItem, i) => 
                        {
                                if (i === index) 
                        ...
                        Alert.alert('Image Duplicated',
                            `You have duplicated image : ${index}`,
                            [{ text: 'OK'}],
                            { cancelable: true }
                ); 
           ...
           <Icon name="close" size={20} color="black" style={{bottom : 5 , right: 10, }}
                onPress={() =>{ 
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.text}>Task 31 — Click Here</Text>
               </View>
